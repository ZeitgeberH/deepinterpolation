...
class core_inferrence_npmm:
    # This is the generic inferrence class working with numpy memmap
    def __init__(
        self, inferrence_json_path: Union[str, Path], generator_obj
    ):
        self.model = None
    ...
    def _create_mmap_datasets(
        ...
        self.mmap_file=np.memmap(self.output_file, dtype=self.output_datatype, mode='w+', shape=tuple(final_shape))
        # with h5py.File(self.output_file, "w") as file_handle:
        #     file_handle.create_dataset(
        #         output_dataset_name,
        #         shape=tuple(final_shape),
        ...
        #             dtype=self.output_datatype,
        #         )
        logger.info(f"Created empty numpy memory map file {self.output_file}")
    ...
    def _write_output_to_file(
        ...
        """Write denoised outputs in the mmap_file
        """
        local_size = corrected_data.shape[0]
        start = self._get_first_output_index() + index_dataset * self.batch_size
        end = start + local_size
        self.mmap_file[start:end] = np.squeeze(corrected_data, -1)
        # with h5py.File(self.output_file, "a") as file_handle:
        #     dset_out = file_handle[self.output_dataset_name]
        #     dset_out[start:end] = np.squeeze(corrected_data, -1)
    ...
    def run(self):
        self.model = _load_model(self.json_data)
        epochs = np.arange(self.nb_datasets)
        nEpochs = len(epochs)
        for epoch_index, index_dataset in enumerate(tqdm(epochs)):
            tqdm.write(f'epoch {epoch_index} of {nEpochs}')
            local_data = self.generator_obj[index_dataset]
            # We overwrite epoch_index to allow the last unfilled epoch
            self.generator_obj.epoch_index = epoch_index
            local_mean, local_std = self.generator_obj.__get_norm_parameters__(
